...
We developed standard answers for a set of 96 predefined questions. Each model has three configurations: (1) a Standard LLM-Based Chatbot, (2) an LLM-Based Chatbot with RAG Integration, and (3) an LLM-Based Chatbot with RAG Integration and a Tailored Response Mechanism. However, upon evaluation, the responses generated by the third configuration (RAG Integration with Tailored Response Mechanism) were identical to the standard answers, rendering this configuration redundant for comparison purposes. Consequently, we focused on the first two configurations, resulting in six distinct responses across the models for each question. To assess and compare these responses, we employed specific evaluation metrics, each assigned a corresponding weight.
...
Reason for the Weights: ROUGE metrics are designed to evaluate recall, which is critical for determining whether the generated response covers all relevant content from the reference response. Here's a breakdown of why these metrics received their specific weights.
